   ...
   kk = probs$pT
    ...
    {  			
      ...
      if(!is.na(b.elim[nc]))
      {
        ...
        {      
          elimi[d:ndose]=1;
          if(d==1) {earlystop=1; break;} else {
            if(sum(elimi[1:d]==0)>0){d_opt=max(which(elimi[1:d]==0));d=d_opt;next;} else {earlystop=1; break}
          }
        }
      ...
      if(max_JUPM<=7 & d!=ndose){
        if(sum(elimi[(d+1):ndose]==0)>0){
          decision=1;d_opt=(d)+min(which(elimi[(d+1):ndose]==0));
        }
      }
      ...
      if(d>1){
        if(max_JUPM==9 | max_JUPM>=13){
          if(sum(elimi[1:(d-1)]==0)>0){
            decision=2;d_opt=max(which(elimi[1:d-1]==0));
          }
        }
      }
      ...
      if(n[d]>=3 && pbeta(lower_e,yE[d]+1,n[d]-yE[d]+1)>pfutility) {
        elimi[d]=1;
        if(decision==0){if(sum(elimi[1:d]==0)>0){d_opt=max(which(elimi[1:d]==0));} else {earlystop=1; break}}
        if(decision==2){if(sum(elimi[1:d]==0)>0){d_opt=max(which(elimi[1:d]==0));} else {earlystop=1; break}}
        if(decision==1){
      ...
      if(((yT[d]/n[d])>target_t)&d_opt>d){incoh<-incoh+1}
      d<-d_opt
    }
    ...
    if (earlystop==0){
	  ...
	  if(pT.true[d_opt]>(target_t+0.1)){ov.sel<-ov.sel+1/ntrial*100}
