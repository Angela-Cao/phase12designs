...
kk = probs$pT
    ...
    {  			
      ...
      if(eff_cut>pfutility) {elimi[d]=1;}
      posH <- posH*(1-elimi);
      
      if (tox.l[tox.ind]>(target_t+0.00001) && d!=1) {
        if(sum(elimi[1:(d-1)]==0)>0){d_opt=max(which(elimi[1:(d-1)]==0))} else {d_opt=d}
      } else if (tox.l[tox.ind]>(target_t+0.00001) && d==1) {if(elimi[d]==0){d_opt=d} else{earlystop=1;break}
      } else{
        admi_set=d;
        if(d>1){
        ...
        if(d<ndose){
          if(safe==0){
            ...
            if(tox.l[tox.ind]<target_t & sum(elimi[(d+1):ndose]==0)>0){admi_set<-c(admi_set,d+min(which(elimi[(d+1):ndose]==0)))}
          }
        }
